...
if find "$downloads_dir" -type f -print -quit | grep -q . || find "$music_dir" -type f -print -quit | grep -q .; then
  ...
  if [ -d "$downloads_dir" ]; then
    ...
    # capitalize accents to lowercase
    ...
    echo -e "[cruix-music-archiver] Oops! Output Directory Not Found: $downloads_dir. Did It Get Lost in the Void? 🌌"
  fi
